...
namespace Uniconv\Converter\Tesseract;

use Uniconv\ConverterInterface;

class TesseractConverter implements ConverterInterface
{
    ...
    public function convertCommand(string $sourceFile, string $targetFile):? string
    {
        ...
        $targetFile = preg_replace('/\\.(pdf|txt)$/i', '', $targetFile);
        $lang = $this->language;
        if (!empty($lang)) {
            $lang = "-l $lang";
        }
    ...
    public static function supportedLanguage(string $lan): bool
    {
        return in_array($lan, array_keys(self::supportedLanguages()));
    }
